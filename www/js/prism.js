...
/* http://prismjs.com/download.html?themes=prism-solarizedlight&languages=markup+css+clike+javascript&plugins=line-numbers */
        ...
        if (t.hasOwnProperty(i) && t[i]) {
          var o = t[i];o = "Array" === n.util.type(o) ? o : [o];for (var s = 0; s < o.length; ++s) {
            var u = o[s],
                ...
                h = !!u.greedy,
                f = 0,
                d = u.alias;h && !u.pattern.global && (u.pattern = RegExp(u.pattern.source, u.pattern.flags + "g")), u = u.pattern || u;for (var p = 0, m = 0; p < r.length; m += (r[p].matchedStr || r[p]).length, ++p) {
              var y = r[p];if (r.length > e.length) break e;if (!(y instanceof a)) {
                u.lastIndex = 0;var v = u.exec(y),
                    b = 1;if (!v && h && p != r.length - 1) {
                  if (u.lastIndex = m, v = u.exec(e), !v) break;for (var k = v.index + (g ? v[1].length : 0), w = v.index + v[0].length, _ = p, P = m, S = r.length; S > _ && w > P; ++_) {
                    P += (r[_].matchedStr || r[_]).length, k >= P && (++p, m = P);
                  }if (r[p] instanceof a || r[_ - 1].greedy) continue;b = _ - p, y = e.slice(m, P), v.index -= m;
                }if (v) {
                  g && (f = v[1].length);var k = v.index + f,
                      v = v[0].slice(f),
                      w = k + v.length,
                      A = y.slice(0, k),
                      x = y.slice(w),
                      O = [p, b];A && O.push(A);var j = new a(i, c ? n.tokenize(v, c) : v, d, v, h);O.push(j), x && O.push(x), Array.prototype.splice.apply(r, O);
                }
              }
            }
          }
